...
def compare(landmarks, prediction, real_points):
    ...
    a = tuple(np.multiply((landmarks[28].x, landmarks[28].y), [1920, 1080]))

    diff_m = math.sqrt((a[0] - real_points[20][0])**2 + (a[1] - real_points[20][1])**2)
    a = tuple(np.multiply(prediction[28], [1920, 1080]))
    diff_k = math.sqrt((a[0] - real_points[20][0])**2 + (a[1] - real_points[20][1])**2)
